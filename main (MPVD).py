...
class TextModel(nn.Module):
    def __init__(self, model_name: str = 'electra', projection_dim = 1024):
        """
        :param model_name: "distilbert", "roberta" 或 "electra"
        """
...
def load_image_model(model_name='resnet_dy_tri', num_classes=6):
...
class ContrastiveLoss(nn.Module):
    def __init__(self, temperature=0.07):
        ...
        self.center_weight = 0.1   # weight for the center-loss term
...
def train_epoch(model, train_loader, optimizer, all_texts, device, class_names, epoch, save_dir):
    ...
    for images, labels in tqdm(train_loader):
        ...
        loss = alpha * classification_loss + (1-alpha) * criterion(anchor_features, positive_text_features, negative_text_features) 
    ...
    plot_train_confusion_matrix(cm, class_names, os.path.join(save_dir, f"train_confusion_matrix_epoch_{epoch + 1}.png"))
...
def evaluate_epoch(model, val_loader, all_texts, device, class_names, epoch, save_dir):
    ...
    with torch.no_grad():
        ...
        for images, labels in tqdm(val_loader, total=len(val_loader)):
            ...
            for i in range(len(images)):
                ...
                all_class_features = F.normalize(all_class_features, p=2, dim=-1)
    ...
    all_features = np.vstack(all_features)  # 转换为二维数组 [total_samples, feature_dim]
    plot_tsne(all_features, all_labels, class_names, os.path.join(save_dir, f"tsne_epoch_{epoch + 1}.png"))
...
def train_model(train_dir, val_dir, test_dir):
    ...
    test_loader = load_data(test_dir, train_dir, opts.batch_size, opts.num_workers)
    ...
    for epoch in range(opts.epochs):
        ...
        if avg_val_acc >= best_acc:
            ...
            best_model_val = copy.deepcopy(image_model.state_dict())
            print(f"✓ New best model found at epoch {epoch+1}, val acc: {best_acc:.2f}%")

        scheduler.step()

    if best_model_val is not None:
        image_model.load_state_dict(best_model_val)
        print(f"✓ Loaded best model with val acc: {best_acc:.2f}% for test evaluation.")    
    test_acc = evaluate_epoch(image_model, test_loader, all_texts, opts.device, class_names, opts.epochs, save_dir)
    print(f"Test Accuracy: {test_acc:.2f}%")

if __name__ == "__main__":
    train_dir = 'E:\mIT-CMCA\dataset\MPVD/train'
    val_dir = 'E:\mIT-CMCA\datamIT-CMCAset\MPVD/val'
    test_dir = 'E:\mIT-CMCA\dataset\MPVD/test'
